...
class rqlite:
    ...
    def init(self):
        ...
        self.execute(["CREATE TABLE geocast (popID INTEGER NOT NULL PRIMARY KEY, FOREIGN KEY(popID) REFERENCES pops(id) ON DELETE CASCADE)"])
        self.execute(["CREATE TABLE domains (domain TEXT NOT NULL PRIMARY KEY, nsv4 TEXT NOT NULL, email TEXT NOT NULL, geocast INTEGER DEFAULT 1)"])
